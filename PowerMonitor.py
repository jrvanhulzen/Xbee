...
def run_main():
    ...
    try:
        ...
        try:
            ...
            mvTempK = (readoutA0)
            tempK = ((mvTempK))/10 # TMP 35 : offset 0 mV Output voltage scaling 10 mV/oC 750 mV @ 25 oC
        ...
        try:
           ...
           # Prepare values for database storage 
            ...
            try:
                ...
                readoutA0 = xbee_message.data[4] << 8 | xbee_message.data[5]
                mvTempK = (readoutA0) # Translate reading in mv to temperature
                tempK = (mvTempK)/10 # TMP 35, offset 0 mV 10 mv/oC
                if debug:
                    print("message = ")
                    print(xbee_message)
            ...
            if logger.handlers[0].level == 10:
                ...
                t.add_row(['Vermogen', round(results['ct4']['power'],1)])
                t.add_row(['Stroom', round(results['ct4']['current'], 3)])
                t.add_row(['Power factor', round(results['ct4']['pf'], 3)])
                t.add_row(['Spanning', round(results['voltage'], 1)])
                t.add_row(['Temperatuur omgeving', round(results['pitemp'], 1)])
                t.add_row(['Temperatuur Koelkast', round(tempK, 1)])
                s = t.get_string()
                logger.debug('\n' + s)
            #sleep(0.1)
